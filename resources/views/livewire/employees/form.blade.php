<div class="mx-auto max-w-2xl">
    <form wire:submit="save" class="space-y-5 rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
        <div class="grid gap-5 sm:grid-cols-2">
            ...
            <div>
                <label class="mb-1 block text-sm font-medium text-stone-700">سمت</label>
                <input type="text" wire:model="role" placeholder="نجار، پرداخت‌کار…" class="w-full rounded-lg border border-stone-300 px-3 py-2 text-sm focus:border-amber-500 focus:ring-amber-500">
            </div>
            <div>
            ...
            <div>
                <label class="mb-1 block text-sm font-medium text-stone-700">نوع معاش</label>
                <select wire:model="salary_type" class="w-full rounded-lg border border-stone-300 px-3 py-2 text-sm focus:border-amber-500 focus:ring-amber-500">
                    @php $salaryTypeLabels = ['monthly' => 'ماهانه', 'daily' => 'روزانه', 'hourly' => 'ساعتی']; @endphp
                    @foreach (\App\Models\Employee::SALARY_TYPES as $t)<option value="{{ $t }}">{{ $salaryTypeLabels[$t] ?? ucfirst($t) }}</option>@endforeach
                </select>
            </div>
            <div>
                <label class="mb-1 block text-sm font-medium text-stone-700">نرخ معاش *</label>
            ...
            <div>
                <label class="mb-1 block text-sm font-medium text-stone-700">تاریخ استخدام</label>
            ...
            <div class="flex items-end">
                <label class="flex items-center gap-2 text-sm text-stone-600"><input type="checkbox" wire:model="is_active" class="rounded border-stone-300 text-amber-600"> فعال</label>
            </div>
        </div>
        <div>
        ...
        <div class="flex items-center justify-end gap-3 border-t border-stone-100 pt-4">
            <a href="{{ route('employees.index') }}" wire:navigate class="rounded-lg px-4 py-2 text-sm font-medium text-stone-600 hover:bg-stone-100">لغو</a>
            <button type="submit" class="rounded-lg bg-amber-600 px-5 py-2 text-sm font-semibold text-white hover:bg-amber-700">ذخیره کارمند</button>
        </div>
    </form>
</div>
