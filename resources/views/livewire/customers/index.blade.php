<div class="space-y-4">
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div class="relative sm:w-80">
            <input type="search" wire:model.live.debounce.300ms="search" placeholder="جستجوی مشتریان…"
        ...
        <table id="customers-table" class="min-w-full divide-y divide-stone-200 text-sm">
            <thead class="bg-stone-50 text-right text-xs font-semibold uppercase tracking-wide text-stone-500">
                <tr>
                    ...
                    <th class="hidden px-4 py-3 md:table-cell">کارها</th>
                ...
                @forelse ($customers as $customer)
                    <tr class="hover:bg-stone-50">
                        ...
                        <td class="px-4 py-3 text-left">
                            <a href="{{ route('customers.edit', $customer) }}" wire:navigate class="text-sm font-medium text-stone-500 hover:text-amber-700">ویرایش</a>
                            <button wire:click="confirmDelete({{ $customer->id }})" class="mr-3 text-sm font-medium text-stone-400 hover:text-red-600">حذف</button>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="4" class="px-4 py-12 text-center text-stone-400">هنوز مشتری‌ای وجود ندارد. اولین مشتری خود را اضافه کنید.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-stone-900/50 p-4">
            <div class="my-4 max-h-[90vh] w-full max-w-sm overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
                ...
                <div class="mt-6 flex justify-end gap-3">
                    <button wire:click="$set('deletingId', null)" class="rounded-lg px-4 py-2 text-sm font-medium text-stone-600 hover:bg-stone-100">لغو</button>
                    <button wire:click="delete" class="rounded-lg bg-red-600 px-4 py-2 text-sm font-semibold text-white hover:bg-red-700">حذف</button>
                </div>
            </div>
        </div>
    @endif
