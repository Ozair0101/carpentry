<div class="mx-auto max-w-3xl">
    <form wire:submit="save" class="space-y-5 rounded-2xl border border-stone-200 bg-white p-6 shadow-sm">
        <div class="grid gap-5 sm:grid-cols-2">
            <div>
                <label class="mb-1 block text-sm font-medium text-stone-700">Customer *</label>
            ...
            <div>
                ...
                <select wire:model="status" class="w-full rounded-lg border border-stone-300 px-3 py-2 text-sm focus:border-amber-500 focus:ring-amber-500">
                    @foreach (\App\Models\Project::STATUSES as $s)<option value="{{ $s }}">{{ ucwords(str_replace('_', ' ', $s)) }}</option>@endforeach
                </select>
            </div>
            ...
            <div class="sm:col-span-2">
                <label class="mb-1 block text-sm font-medium text-stone-700">Site address</label>
            ...
            <div>
                <label class="mb-1 block text-sm font-medium text-stone-700">Assigned to</label>
                <input type="text" wire:model="assigned_to" placeholder="e.g. James (lead)" class="w-full rounded-lg border border-stone-300 px-3 py-2 text-sm focus:border-amber-500 focus:ring-amber-500">
            </div>
            <div>
                <label class="mb-1 block text-sm font-medium text-stone-700">Budget</label>
        ...
        <div class="flex items-center justify-end gap-3 border-t border-stone-100 pt-4">
            <a href="{{ route('jobs.index') }}" wire:navigate class="rounded-lg px-4 py-2 text-sm font-medium text-stone-600 hover:bg-stone-100">Cancel</a>
            <button type="submit" class="rounded-lg bg-amber-600 px-5 py-2 text-sm font-semibold text-white hover:bg-amber-700">Save job</button>
        </div>
    </form>
</div>
