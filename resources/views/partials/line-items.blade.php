    ...
    <div class="overflow-x-auto">
        <table class="min-w-full text-sm">
            <thead class="text-left text-xs font-semibold uppercase tracking-wide text-stone-400">
                <tr>
                    <th class="pb-2 pr-2">Item / description</th>
                    <th class="pb-2 px-2 w-24">Qty</th>
                    <th class="pb-2 px-2 w-24">Unit</th>
                    <th class="pb-2 px-2 w-32">Unit price</th>
                    <th class="pb-2 px-2 w-32 text-right">Total</th>
                ...
                @foreach ($items as $index => $item)
                    <tr wire:key="item-{{ $index }}" class="align-top">
                        <td class="py-1 pr-2">
                            ...
                            <input type="text" wire:model="items.{{ $index }}.description" placeholder="Description"
                        ...
                        <td class="py-1 px-2 pt-3 text-right font-medium text-stone-700">{{ Format::money($this->lineTotal($item)) }}</td>
                        <td class="py-1 pl-2 pt-2 text-center">
                            <button type="button" wire:click="removeItem({{ $index }})" class="text-stone-400 hover:text-red-600" title="Remove">✕</button>
                        </td>
                    </tr>
                @endforeach
            </tbody>
    ...
    <div class="mt-5 flex justify-end">
        <div class="w-full space-y-2 sm:w-72">
            ...
            <div class="flex items-center justify-between text-sm">
                ...
                <input type="number" step="0.01" wire:model.live="discount" class="w-28 rounded-lg border border-stone-300 px-2 py-1 text-right text-sm">
            </div>
            <div class="flex items-center justify-between text-sm">
                <span class="text-stone-500">Tax rate %</span>
                <input type="number" step="0.01" wire:model.live="tax_rate" class="w-28 rounded-lg border border-stone-300 px-2 py-1 text-right text-sm">
            </div>
            <div class="flex justify-between text-sm">
